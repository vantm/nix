{ pkgs, lib, config, ... }:
let
  swayModules = lib.mkIf config.wayland.windowManager.sway.enable {
    "modules-left" = [ "sway/mode" "sway/workspaces" ];
    "sway/mode" = { };
    "sway/scratchpad" = { "format-icons" = [ "󰹕" ]; };
    "sway/workspaces" = { };
  };
  hyprlandModules = lib.mkIf config.wayland.windowManager.hyprland.enable {
      ...
      (lib.mkMerge [
        {
          ...
          "modules-right" = (if config.wayland.windowManager.sway.enable then [
            "sway/scratchpad"
          ] else [ ]) ++ [
            "cpu"
            "memory"
